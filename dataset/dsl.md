...
shape_decl     ::= "triangle" "(" POINT "," POINT "," POINT ")"
                 ...
                 | "polygon" "(" POINT ("," POINT)+ ")"
                 | "regular_polygon" "(" NUMBER "," POINT ("," POINT)+ ")"
...
relation_constraint ::= "parallel" "(" LINE "," LINE ")"
                      ...
                      | "incenter" "(" POINT "," POINT "," POINT "," POINT ")"      (* v1.8.8: 内心 *)
                      | "centroid" "(" POINT "," POINT "," POINT "," POINT ")"      (* v1.8.8: 重心 *)
                      | "orthocenter" "(" POINT "," POINT "," POINT "," POINT ")"   (* v1.8.8: 垂心 *)
...
measure_constraint ::= "angle" "(" ANGLE ")" "=" NUMBER
                     ...
                     | "distance" "(" SEGMENT "," SEGMENT ")" "=" NUMBER  (* v1.8.5: 平行线段间距离 *)
...
macro_stmt     ::= "isosceles" "(" POINT "," POINT "," POINT ")"
                 ...
                 | "rotate_to" "(" POINT "," "source" "=" POINT "," "center" "=" POINT ","
                   "target" "=" SEGMENT ("," "direction" "=" DIRECTION_VAL)? ")"

DIRECTION_VAL  ::= "cw" | "ccw"
...
region_stmt    ::= "inside" "(" POINT "," REGION_REF ")"
                 ...
                 | "opposite_sides" "(" POINT "," POINT "," SEGMENT ("," "strict" "=" "true")? ")"
                 | "same_sides" "(" POINT "," POINT "," SEGMENT ("," "strict" "=" "true")? ")"
                 | "shaded" "(" REGION_REF ")"
...
regular_polygon(5, A, B, C, D, E)  # 正五边形（n=5，顶点按顺序）
...
regular_polygon(5, A, B, C, D, E)  # 正五边形（n 与顶点数必须一致）
```

### 7.1.1 正多边形声明

```text
regular_polygon(5, A, B, C, D, E)   # 正五边形
regular_polygon(6, A, B, C, D, E, F) # 正六边形
```

**语义**：

- 第一个参数 `n` 是边数，后续按顺序给出 `n` 个顶点
- 隐含约束：所有边等长，所有内角相等
- `n` 必须是整数且 `n >= 3`
- 顶点数量必须与 `n` 完全一致
...
- 「正五边形 ABCDE」→ `regular_polygon(5, A, B, C, D, E)`
- 「正 n 边形」→ `regular_polygon(n, ...)`（标注时需替换为具体数字）
...
### 8.6 旋转至目标（rotate_to）(v1.8.4)

```text
rotate_to(A1, source=A, center=C, target=BE)
rotate_to(A1, source=A, center=C, target=BE, direction=cw)
rotate_to(A1, source=A, center=C, target=BE, direction=ccw)
```
...
**语义**：`rotate_to` 是宏指令，在 Extractor 阶段展开为基础约束。数学上等价于求"以 center 为圆心、|center-source| 为半径的圆与目标线段 target 的交点"。

**与 `rotate` 的本质区别**：

| 指令 | 性质 | 角度 | Layer |
|------|------|------|-------|
| `rotate(D, source=C, center=O, angle=60)` | 确定性变换，2个方程完全确定D | 必须显式指定 | Layer 6 |
| `rotate_to(A1, source=A, center=C, target=BE)` | 宏展开为约束组合，A1由求解器确定 | 隐式确定 | Layer 5 |

**与"连续旋转"的语义差异**：

- `rotate_to` 的数学本质是**圆 ∩ 线段**的交点，不是"从 source 出发连续旋转扫过的第一个交点"
- 当圆与 target 有两个交点时，`direction` 参数用于初始化引导 solver 收敛到期望的交点
- `direction` 是**软约束/初始化提示**（与 `inside`、`half_plane` 等消歧指令一致），不是硬约束
- 在绝大多数几何题场景中，圆与 target 只有一个交点，不存在歧义

**`direction` 参数说明**：

- `direction=cw`：顺时针方向（从 source 到 result 的旋转方向为顺时针）
- `direction=ccw`：逆时针方向
- 省略时：不指定方向偏好，solver 取最近的有效交点

**对应中文**：

- 「将线段 CA 绕点 C 按顺时针方向旋转，点 A 落在边 BE 上记为 A1」→ `rotate_to(A1, source=A, center=C, target=BE, direction=cw)`
- 「以 C 为圆心、CA 为半径画弧，交 BE 于点 A1」→ `rotate_to(A1, source=A, center=C, target=BE)`
...
```text
circumcircle(O, A, B, C)    # O 是 △ABC 的外心（已有，v1.5.2）
incenter(I, A, B, C)        # I 是 △ABC 的内心
centroid(G, A, B, C)        # G 是 △ABC 的重心
...
| 关键字 | 几何定义 | 约束本质 |
| --- | --- | --- |
| `circumcircle(O, A, B, C)` | O 是三边垂直平分线的交点 | \|OA\| = \|OB\| = \|OC\| |
| `incenter(I, A, B, C)` | I 是三个内角角平分线的交点 | dist(I, AB) = dist(I, BC) = dist(I, CA) |
| `centroid(G, A, B, C)` | G 是三条中线的交点 | G = (A + B + C) / 3 |
| `orthocenter(H, A, B, C)` | H 是三条高的交点 | HA ⊥ BC 且 HB ⊥ AC |

**对应中文**：

- 「O 是 △ABC 的外心」→ `circumcircle(O, A, B, C)`
- 「I 是 △ABC 的内心」→ `incenter(I, A, B, C)`
- 「G 是 △ABC 的重心」→ `centroid(G, A, B, C)`
- 「H 是 △ABC 的垂心」→ `orthocenter(H, A, B, C)`
- 「O 是 △ABC 的外接圆圆心」→ `circumcircle(O, A, B, C)`
- 「I 是 △ABC 的内切圆圆心」→ `incenter(I, A, B, C)`

**等价展开形式**（理解用，实际不展开）：

- `incenter(I, A, B, C)` ≡ `angle_bisector` × 3 + `intersection`
- `centroid(G, A, B, C)` ≡ `midpoint` × 3 + `intersection`
- `orthocenter(H, A, B, C)` ≡ `foot` × 3 + `intersection`

**拓扑排序**：归入 Layer 4（位置约束 — 三角形中心子类别）
...
```text
distance(AB, CD)=2          # 平行线段 AB 与 CD 之间的垂直距离为 2
```

**语义**：线段 AB 所在直线与线段 CD 所在直线之间的垂直距离为 d。

**前提条件**：两线段已声明 `parallel(AB, CD)`，否则几何意义不明确。Extractor 会在缺少 `parallel` 声明时发出语义警告。

**约束方程（双点约束）**：对 AB 的两个端点 A、B 分别计算到直线 CD 的垂直距离，两者均应等于 d。这种设计在数值求解中间态（AB 和 CD 尚未严格平行时）提供更好的梯度引导。

**对应中文**：

| 中文表达 | DSL |
|---------|-----|
| AB 与 CD 之间的距离为 2 | `distance(AB, CD)=2` |
| 两平行线之间的距离为 h | `distance(AB, CD)=h`（h 为具体数值） |
| 梯形的高为 3 | `distance(AB, CD)=3`（AB、CD 为两底） |

**备注**：未来可扩展为 `distance(P, AB)=val` 表达点到直线距离，当前版本仅支持 `distance(segment, segment)=val` 形式。
...
#### 14.3.5 异侧/同侧约束 `opposite_sides` / `same_sides` (v1.8.6)

```text
opposite_sides(A, D, PQ)                # A、D 在直线 PQ 异侧（软约束）
opposite_sides(A, D, PQ, strict=true)   # A、D 在直线 PQ 异侧（硬约束）
same_sides(A, D, PQ)                    # A、D 在直线 PQ 同侧（软约束）
same_sides(A, D, PQ, strict=true)       # A、D 在直线 PQ 同侧（硬约束）
```

**语义**：声明两个点相对于一条直线的位置关系。

- **语义基于直线 line(P, Q)，而非有限线段 segment(PQ)**
- 判定依据为叉积符号：`cross = (point - P) x (Q - P)`
- `opposite_sides`：两点的叉积符号相反（`cross_A * cross_D < 0`）
- `same_sides`：两点的叉积符号相同（`cross_A * cross_D > 0`）
- **点在直线上（`|cross| < EPS`）不属于任一侧**，两种约束均视为不满足

**软约束与硬约束**：

- 默认为**软约束**（Layer 9 消歧），仅影响初始化坐标选择，与 `half_plane`/`inside`/`direction` 一致
- 加 `strict=true` 升级为**硬约束**，通过 `compute_error()` 惩罚违反，直接参与数值求解
- 硬约束 `to_equations()` 返回空（不等式无法表示为方程），仅通过数值优化器执行

**退化保护**：当 P == Q 时直线退化，约束无意义，返回不满足/大惩罚值。

**对应中文**：

- 「点 A、D 在 PQ 异侧」→ `opposite_sides(A, D, PQ)`
- 「A、D 分居 BC 两侧」→ `opposite_sides(A, D, BC)`
- 「使点 A、D 在 PQ 异侧」→ `opposite_sides(A, D, PQ)` 或 `opposite_sides(A, D, PQ, strict=true)`
- 「A、D 在 BC 同侧」→ `same_sides(A, D, BC)`

**与 `half_plane` 的区别**：

| 特性 | `half_plane` | `opposite_sides` / `same_sides` |
|------|-------------|-------------------------------|
| 参数 | 1 个点 + 方向 | 2 个点，无需指定方向 |
| 语义 | 绝对位置（left/right） | 相对位置关系 |
| 约束类型 | 仅软约束 | 软/硬可选 |

#### 14.3.6 阴影区域标记 `shaded`
...
| `opposite_sides`            | 默认 soft constraint；`strict=true` 时添加硬约束 |
| `same_sides`                | 默认 soft constraint；`strict=true` 时添加硬约束 |
...
| A、D 分居 PQ 两侧    | `opposite_sides(A, D, PQ)`                    |
| A、D 在 PQ 异侧      | `opposite_sides(A, D, PQ)`                    |
| A、D 在 PQ 同侧      | `same_sides(A, D, PQ)`                        |
| 在 AB 上任取一点 P   | `choose(P, on=AB)`                            |
...
| 将 A 绕 C 旋转至落在 BE 上，记为 A1 | `rotate_to(A1, source=A, center=C, target=BE)` |
| 将 CA 绕 C 顺时针旋转，A 落在 BE 上记为 A1 | `rotate_to(A1, source=A, center=C, target=BE, direction=cw)` |
...
| V5a      | 正多边形边数必须为整数且 ≥ 3，且与顶点数量一致         | `regular_polygon(5, A, B, C, D, E)` ✓, `regular_polygon(5.5,...)` ✗ |
...
| 2    | 图形声明       | `circle(O)`, `triangle(A, B, C)`, `regular_polygon(5, A, B, C, D, E)`, `line_function(l, 2, 1)`                                                          |
| 3    | 线段/射线/直线 | `segment(AB)`, `ray(A, B)`, `line(l, A, B)`                                                                                                                |
| 4    | 位置约束       | `on(D, BC)`, `midpoint(M, AB)`, `foot(H, A, BC)`, `intersection(...)`, `incenter(I, A, B, C)`, `centroid(G, A, B, C)`, `orthocenter(H, A, B, C)`, `circumcircle(O, A, B, C)` |
| 5    | 构造操作/宏    | `extend(AB, D)`, `draw_perpendicular(...)`, `rotate_to(...)`                                                                                                 |
| 6    | 变换操作       | `rotate(...)`, `reflect(...)`, `fold(...)`                                                                                                                 |
| 7    | 几何关系       | `equal(AB, CD)`, `parallel(...)`, `perpendicular(...)`, `inscribed(...)`, `circumscribed(...)`, `similar(...)`, `congruent(...)`, `tangent(...)` |
| 8    | 度量约束       | `angle(ABC)=60`, `length(AB)=5`, `ratio(...)=2`, `distance(AB, CD)=2`                                                                                      |
| 9    | 消歧/辅助      | `inside(...)`, `shaded(...)`, `direction(...)`, `opposite_sides(...)`, `same_sides(...)`                                                                   |
...
- `regular_polygon(5, A, B, C, D, E)` - 声明存在一个正多边形并隐含等边等角约束
...
- `incenter(I, A, B, C)` → 等价于三条 `angle_bisector` + `intersection`，但作为直接约束实现 (Layer 4)
- `centroid(G, A, B, C)` → 等价于三条 `midpoint` + `intersection`，但作为直接约束实现 (Layer 4)
- `orthocenter(H, A, B, C)` → 等价于三条 `foot` + `intersection`，但作为直接约束实现 (Layer 4)

这些 macro 可以放在 Layer 2，因为它们的主要作用是声明图形，附带的约束会自动展开到正确的层级。三角形四心（circumcircle, incenter, centroid, orthocenter）归入 Layer 4 作为位置约束。
...
| `rotate_to(A1, source=A, center=C, target=BE)` | `equal(CA1, CA)` + `on(A1, BE)`                      |
...
- 新增 `incenter(I, A, B, C)` 语法：I 是 △ABC 的内心（三角内角平分线交点）
- 新增 `centroid(G, A, B, C)` 语法：G 是 △ABC 的重心（三条中线交点）
- 新增 `orthocenter(H, A, B, C)` 语法：H 是 △ABC 的垂心（三条高的交点）
- 三者与已有 `circumcircle(O, A, B, C)` 共同构成完整的三角形四心体系
- 引入 `TriangleCenterConstraint` 基类，统一四心约束架构
- 约束公式设计兼顾符号可解性与数值稳定性（`to_equations` 与 `compute_error` 分离）
- 拓扑排序归入 Layer 4（位置约束 — 三角形中心子类别）
- 新增 `INCENTER`、`CENTROID`、`ORTHOCENTER` token
- 新增 `IncenterDecl`、`CentroidDecl`、`OrthocenterDecl` AST 节点
- 新增 `IncenterConstraint`、`CentroidConstraint`、`OrthocenterConstraint` 约束类

#### v1.8.7 (2026-03-03)

**regular_polygon 规范补齐（文档与实现统一）**：

- 在 EBNF `shape_decl` 中补充 `regular_polygon(NUMBER, POINT...)` 产生式
- 新增第 7.1.1 节，明确 `regular_polygon` 的语义与参数约束
  - `n` 必须为整数且 `>= 3`
  - 顶点数量必须与 `n` 一致
  - 隐含等边与等角约束
- 在第 15 章中文对照表补充“正五边形”映射
- 在第 16 章合法性规则补充 V5a（正多边形专项规则）
- 在第 17 章拓扑排序与 Layer 2 示例中明确 `regular_polygon` 属于图形声明

#### v1.8.6 (2026-03-03)

**opposite_sides / same_sides 异侧同侧约束 (新增)**：

- 新增 `opposite_sides(A, D, PQ)` 语法：声明两点在直线 PQ 异侧
- 新增 `same_sides(A, D, PQ)` 语法：声明两点在直线 PQ 同侧
- 默认为软约束（Layer 9 消歧），可选 `strict=true` 升级为硬约束
- 语义基于直线 line(P, Q)，非有限线段
- 点在直线上视为不属于任一侧（EPS 边界策略）
- 退化直线保护（P == Q 时返回大惩罚值）
- 硬约束通过 `compute_error()` 惩罚实现，`to_equations()` 返回空
- 主动初始化优化：extractor 检测到不满足时镜像翻转点坐标
- 新增 `OPPOSITE_SIDES`、`SAME_SIDES`、`STRICT` token
- 新增 `OppositeSidesConstraint`、`SameSidesConstraint` AST 节点
- 新增软约束类 `OppositeSidesSoftConstraint`、`SameSidesSoftConstraint`
- 新增硬约束类 `OppositeSidesHardConstraint`、`SameSidesHardConstraint`

#### v1.8.5 (2026-03-02)

**distance 平行线段间距离约束 (新增)**：

- 新增 `distance(AB, CD)=d` 语法，用于表达两条平行线段之间的垂直距离
- 双点约束设计：同时约束 AB 的两个端点到直线 CD 的距离，增强数值求解稳定性
- 退化保护：线段退化为点时返回大惩罚值
- Extractor 语义检查：缺少 `parallel` 声明时发出 `warnings.warn()`
- 拓扑排序归入 Layer 8（度量约束）
- 新增 `DISTANCE` token
- 新增 `DistanceConstraint` AST 节点
- 新增 `ParallelDistanceConstraint` 约束类
- 扩展性预留：parser 结构支持未来 `distance(P, AB)=val` 形式

#### v1.8.4 (2026-02-28)

**rotate_to 旋转至目标宏指令 (新增)**：

- **新增 `rotate_to(A1, source=A, center=C, target=BE)` 宏指令**：表达"将某点绕中心旋转至落在指定线段上"
  - 宏指令（非几何变换），在 Extractor 阶段 desugar 为基础约束
  - 展开为 `EqualLengthConstraint`（距离保持）+ `PointOnSegmentConstraint`（目标位置）
  - 数学语义：圆（圆心=center，半径=|center-source|）∩ 线段（target）的交点
  - 可选 `direction=cw|ccw` 参数：软约束/初始化提示，不是硬约束
  - 拓扑排序归入 Layer 5（构造/宏操作）
  - 新增 EBNF `macro_stmt` 产生式
  - 新增 `ROTATE_TO`、`TARGET` token
  - 新增 `RotateToConstraint` AST 节点
  - 对应中文：「将线段 CA 绕 C 旋转，A 落在 BE 上记为 A1」
